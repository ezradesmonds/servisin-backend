...
class TechnicianMatchingService
{
    public function recommendedForCategory(int $categoryId, int $limit = 10)
    {
        return DB::table('technician_profiles')
            ...
            ->select('technician_profiles.*', 'users.id', 'users.name', 'users.avatar', 'technician_services.min_price', 'technician_services.max_price', 'technician_services.diagnosis_fee')
